function clearTimeEstimates() {
	$('.manually-entered-time, .dxf-time-estimate').html('');
	$('.manual-time-estimate .fa').addClass('fa-circle-o').removeClass('fa-check-circle');
	$('.job-file-upload .fa').addClass('fa-circle-o').removeClass('fa-check-circle');
}

$(queueEvents).on('form.reset', clearTimeEstimates);

acceptedAPIs.dxfte_send_materials = [function(materials) {
	materials = materials.materials;
	// add API for parsing time estimate
	acceptedAPIs.dxf_estimate = [function writeEstimate(data){
		NProgress.done();
		$('.job-time-estimate').val(data.time);
		clearTimeEstimates();
		$('.dxf-time-estimate').html(' ({time})'.format({time:prettyTime(data.time)}));

		// move check indicator to file upload
		$('.job-file-upload .time-usage-indicator').removeClass('fa-circle-o').addClass('fa-check-circle');
		$('.manual-time-estimate .time-usage-indicator').addClass('fa-circle-o').removeClass('fa-check-circle');
	}]

	// once config has been applied
	getConfigFile.done(function() {
		// create the file uploader
		formOptions.dxfTime = {
			"type": "generic",
			"noParse": true,
			"template":
			"<div class='btn-group' role='group'>" +
				"<label type='button' for='dxf-file-picker' class='btn dxf-file-picker-label'>" +
					"<span data-form-el-type='file' class='form-control btn-default form-simple-button job-file-upload' data-toggle='tooltip' data-placement='bottom' title='Upload DXF to get automatic time estimate'>" +
						"<i class='fa fa-circle-o time-usage-indicator'></i>Upload DXF <span class='dxf-time-estimate'></span><input type='file' id='dxf-file-picker'>" +
					"</span>" +
				"</label>" +
				"<button type='button' class='btn btn-default form-simple-button manual-time-estimate' data-toggle='tooltip' data-placement='bottom' title='Enter cut time estimate manually'><i class='fa fa-circle-o time-usage-indicator'></i>Manual time estimate <span class='manually-entered-time'></span></button>" +
			"</div>",
			"onRender": function() {
				// hide normal time estimate box and move button to right place
				$('.job-time-estimate').hide();
				// when the file is chosen
				$('#dxf-file-picker').change(function logData(data) {
					var file = $(this).get(0).files[0];
					var read = new FileReader();
					read.readAsBinaryString(file);
					read.onloadend = function transferFile() {
						if($('.job-material').val() in materials) {
							// send it to the backend
							socketSend({
								"action": "parse_dxf",
								"material": $('.job-material').val(),
								"name": $('.job-human-name').val(),
								"dxf_data": read.result
							});
						} else {
							modalMessage('Material speed',
								'<form class="matspeed-form">' +
									'<div class="form-group">' +
										'<label for="matspeed-input">Material cut speed</label>' +
										'<input type="text" class="form-control pink-textbox matspeed-input" id="matspeed-input" placeholder="Material cut speed">' +
									'</div>' +
									'<button type="submit" class="btn btn-pink matspeed-submit">OK</button>' +
								'</form>'
							);

							$('.matspeed-submit').click(function(event) {
								event.preventDefault();
								$('.modal').modal('hide');
								// send it to the backend
								socketSend({
									"action": "parse_dxf_customspeed",
									"material_speed": +$('.matspeed-input').val(),
									"name": $('.job-human-name').val(),
									"dxf_data": read.result
								});
							});
						}
					}
					// start a loader
					NProgress.start();
				});
				// when user wants to enter manual estimate
				$('.manual-time-estimate').click(function() {
					modalMessage('Custom cut time',
						'<form class="cut-time-form">' +
							'<div class="form-group">' +
								'<label for="cut-time-modal-input">Time</label>' +
								'<input type="number" class="form-control form-pink-input cut-time-modal-input" id="cut-time-modal-input" placeholder="Time (minutes)">' +
							'</div>' +
							'<button type="submit" class="btn btn-pink cut-time-modal-confirm">OK</button>' +
						'</form>'
					);

					// once modal is up, focus time
					setTimeout(function focusTimeField() {
						$(".cut-time-modal-input").focus();
					}, 500);

					// when user clicks OK
					$('.cut-time-modal-confirm').click(function(event) {
						event.preventDefault();
						$("#notify-modal").modal("hide");
						clearTimeEstimates();
						// fill in time
						$('.job-time-estimate').val($('.cut-time-modal-input').val());
						$('.manually-entered-time').html(
							'({time})'.format({time:prettyTime($('.cut-time-modal-input').val())})
						);
						// move check indicator to manual
						$('.job-file-upload .time-usage-indicator').addClass('fa-circle-o').removeClass('fa-check-circle');
						$('.manual-time-estimate .time-usage-indicator').removeClass('fa-circle-o').addClass('fa-check-circle');
					});
				});
			}
		}
		renderForm();
	});

	function prettyTime(minutes) {
		return prettifyTime(
			Math.round(minutes * 100) / 100
		);
	}
}]
